...
export class SemanticDriftTool {
  ...
  public performDrift(input: SemanticDriftInput): SemanticDriftOutput {
    ...
    const calibratedMagnitude = this.calibrateDriftMagnitude(
      driftMagnitude,
      temperature,
    );
    ...
    const crossDomainChance = Math.max(
      0.1,
      Math.min(0.5, calibratedMagnitude * 0.5),
    );
  ...
  private findNextConcept(
    ...
    let targetMaxDistance = Math.min(
      1.0,
      driftMagnitude + targetBandwidth * 0.5,
    );
    ...
    if (temperature < 0.3) {
      ...
      const halfBand =
        ((targetMaxDistance - targetMinDistance) / 2) * tightening;
