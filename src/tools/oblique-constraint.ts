...
export class ObliqueConstraintTool {
  ...
  public generateConstraint(
    ...
    if (constraintType === "random") {
      ...
      switch (constraintType) {
        case "oblique":
          selectedConstraint = this.selectBestConstraint(
            this.strategies,
            blockAnalysis,
          );
  ...
  private analyzeBlock(block: string): BlockAnalysis {
    ...
    const technicalScore = technicalTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const creativeScore = creativeTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const strategicScore = strategicTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const interpersonalScore = interpersonalTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    ...
    const stuckScore = stuckTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const exploringScore = exploringTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const decidingScore = decidingTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
    const executingScore = executingTerms.filter((term) =>
      lowerBlock.includes(term),
    ).length;
   ...
   */
  private selectContextAwareConstraint(analysis: BlockAnalysis): {
    constraint: string;
    type: "oblique" | "scamper" | "creative";
  } {
  ...
  private selectBestConstraint(
    ...
    const topCandidates = scoredConstraints.slice(
      0,
      Math.min(3, constraints.length),
    );
  ...
  private scoreConstraintRelevance(
    ...
    switch (analysis.problemType) {
      case "technical":
        if (
          lowerConstraint.includes("system") ||
      ...
      case "creative":
        if (
          lowerConstraint.includes("imagine") ||
      ...
      case "strategic":
        if (
          lowerConstraint.includes("reverse") ||
      ...
      case "interpersonal":
        if (
          lowerConstraint.includes("friend") ||
    ...
    switch (analysis.sentiment) {
      case "stuck":
        if (
          lowerConstraint.includes("error") ||
      ...
      case "exploring":
        if (
          lowerConstraint.includes("what if") ||
          lowerConstraint.includes("adapt") ||
      ...
      case "deciding":
        if (
          lowerConstraint.includes("choose") ||
      ...
      case "executing":
        if (
          lowerConstraint.includes("simple") ||
  ...
  private generateSpecificApplicationHints(
    ...
    if (
      ...
      hints.push(
        `In your ${analysis.problemType} problem, try replacing your core approach with something completely different`,
      );
      if (analysis.keywords.length > 0) {
        const keyword = analysis.keywords[0];
        hints.push(
          `What if you substituted "${keyword}" with its opposite or inverse?`,
        );
      }
    } else if (
      lowerConstraint.includes("combine") ||
      ...
      hints.push(
        `Look for two contradictory elements in your ${analysis.problemType} challenge and merge them`,
      );
      if (analysis.keywords.length >= 2) {
        hints.push(
          `Try combining "${analysis.keywords[0]}" and "${analysis.keywords[1]}" in an unexpected way`,
        );
      }
    } else if (
      lowerConstraint.includes("remove") ||
      ...
      hints.push(
        `What would happen if you removed the element you consider most essential?`,
      );
      if (analysis.keywords.length > 0) {
        hints.push(
          `Try solving this without using "${analysis.keywords[0]}" at all`,
        );
      }
    } else if (
      lowerConstraint.includes("reverse") ||
      ...
      hints.push(
        `Reverse the order or priority of your ${analysis.problemType} approach`,
      );
      ...
      hints.push(
        `How might you adapt a solution from a completely different domain?`,
      );
      if (analysis.problemType === "technical") {
        hints.push(
          "Could you borrow an approach from nature, art, or social systems?",
        );
      }
    } else if (
      lowerConstraint.includes("simplif") ||
      ...
      hints.push(
        `Apply this constraint to your ${analysis.problemType} problem literally`,
      );
      hints.push(
        `Use this as a metaphor for your current ${analysis.sentiment} state`,
      );
    }
    ...
    switch (analysis.sentiment) {
      case "stuck":
        hints.push(
          "This constraint is designed to break you out of your current impasse",
        );
        break;
      case "exploring":
        hints.push(
          "Use this to expand your exploration in an unexpected direction",
        );
        break;
      case "deciding":
        hints.push(
          "Apply this constraint to reveal which option has more creative potential",
        );
        break;
      case "executing":
    ...
    if (analysis.problemType === "technical") {
      hints.push(
        "Consider how this constraint might expose architectural assumptions",
      );
    } else if (analysis.problemType === "creative") {
      hints.push(
        "Let this constraint guide you toward more innovative expressions",
      );
      ...
      hints.push(
        "Apply this to improve communication and collaboration dynamics",
      );
    }
