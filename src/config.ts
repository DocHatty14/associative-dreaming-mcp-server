...
export interface SerendipityScanConfig {
  ...
  bridgeBoost: number;
  temporalDiversityWindow: number; // minutes
  maxResults: number;
}
 ...
 */
function loadFromEnv(defaults: DreamConfig): DreamConfig {
  return {
    semanticDrift: {
      defaultTemperature: parseNumber(
        process.env.DRIFT_TEMPERATURE,
        defaults.semanticDrift.defaultTemperature,
      ),
      defaultDriftMagnitude: parseNumber(
        process.env.DRIFT_MAGNITUDE,
        defaults.semanticDrift.defaultDriftMagnitude,
      ),
      targetBandwidth: parseNumber(
        process.env.DRIFT_BANDWIDTH,
        defaults.semanticDrift.targetBandwidth,
      ),
      maxDriftIterations: parseNumber(
        process.env.DRIFT_MAX_ITERATIONS,
        defaults.semanticDrift.maxDriftIterations,
      ),
      crossDomainChance: parseNumber(
        process.env.DRIFT_CROSS_DOMAIN_CHANCE,
        defaults.semanticDrift.crossDomainChance,
      ),
      loopDetectionEnabled: parseBoolean(
        process.env.DRIFT_LOOP_DETECTION,
        defaults.semanticDrift.loopDetectionEnabled,
      ),
    },
    serendipityScan: {
      defaultNoveltyThreshold: parseNumber(
        process.env.SERENDIPITY_NOVELTY,
        defaults.serendipityScan.defaultNoveltyThreshold,
      ),
      recentHistoryWindow: parseNumber(
        process.env.SERENDIPITY_HISTORY_WINDOW,
        defaults.serendipityScan.recentHistoryWindow,
      ),
      bridgeBoost: parseNumber(
        process.env.SERENDIPITY_BRIDGE_BOOST,
        defaults.serendipityScan.bridgeBoost,
      ),
      temporalDiversityWindow: parseNumber(
        process.env.SERENDIPITY_TEMPORAL_WINDOW,
        defaults.serendipityScan.temporalDiversityWindow,
      ),
      maxResults: parseNumber(
        process.env.SERENDIPITY_MAX_RESULTS,
        defaults.serendipityScan.maxResults,
      ),
    },
    bisociativeSynthesis: {
      defaultBlendIntensity: parseNumber(
        process.env.SYNTHESIS_BLEND_INTENSITY,
        defaults.bisociativeSynthesis.defaultBlendIntensity,
      ),
      maxMappings: parseNumber(
        process.env.SYNTHESIS_MAX_MAPPINGS,
        defaults.bisociativeSynthesis.maxMappings,
      ),
      preferredPatterns: parseArray(
        process.env.SYNTHESIS_PATTERNS,
        defaults.bisociativeSynthesis.preferredPatterns,
      ),
    },
    obliqueConstraint: {
      maxHints: parseNumber(
        process.env.OBLIQUE_MAX_HINTS,
        defaults.obliqueConstraint.maxHints,
      ),
      preferContextual: parseBoolean(
        process.env.OBLIQUE_PREFER_CONTEXTUAL,
        defaults.obliqueConstraint.preferContextual,
      ),
    },
    metaAssociation: {
      defaultChaosIntensity: parseNumber(
        process.env.META_CHAOS_INTENSITY,
        defaults.metaAssociation.defaultChaosIntensity,
      ),
      minPriorOutputs: parseNumber(
        process.env.META_MIN_OUTPUTS,
        defaults.metaAssociation.minPriorOutputs,
      ),
      maxCollisionPoints: parseNumber(
        process.env.META_MAX_COLLISIONS,
        defaults.metaAssociation.maxCollisionPoints,
      ),
      weirdnessThreshold: parseNumber(
        process.env.META_WEIRDNESS_THRESHOLD,
        defaults.metaAssociation.weirdnessThreshold,
      ),
    },
    graph: {
      clusterWeightThreshold: parseNumber(
        process.env.GRAPH_CLUSTER_THRESHOLD,
        defaults.graph.clusterWeightThreshold,
      ),
      betweennessSampling: parseBoolean(
        process.env.GRAPH_BETWEENNESS_SAMPLING,
        defaults.graph.betweennessSampling,
      ),
      betweennessSampleSize: parseNumber(
        process.env.GRAPH_SAMPLE_SIZE,
        defaults.graph.betweennessSampleSize,
      ),
      maxShortestPathDepth: parseNumber(
        process.env.GRAPH_MAX_PATH_DEPTH,
        defaults.graph.maxShortestPathDepth,
      ),
      cacheEnabled: parseBoolean(
        process.env.GRAPH_CACHE_ENABLED,
        defaults.graph.cacheEnabled,
      ),
      cacheTTL: parseNumber(
        process.env.GRAPH_CACHE_TTL,
        defaults.graph.cacheTTL,
      ),
    },
    logging: {
      enabled: !parseBoolean(
        process.env.DISABLE_DREAM_LOGGING,
        !defaults.logging.enabled,
      ),
      level:
        (process.env.LOG_LEVEL as DreamConfig["logging"]["level"]) ||
        defaults.logging.level,
      colorized: parseBoolean(
        process.env.LOG_COLORIZED,
        defaults.logging.colorized,
      ),
      includeTimestamp: parseBoolean(
        process.env.LOG_TIMESTAMP,
        defaults.logging.includeTimestamp,
      ),
    },
  };
}
 ...
 */
function loadFromFile(filePath: string, defaults: DreamConfig): DreamConfig {
  try {
...
let configInstance: DreamConfig | null = null;

/**
 * Load and return the configuration
 * Priority: Environment variables > config file > defaults
 */
export function loadConfig(configFilePath?: string): DreamConfig {
  ...
  const filePath =
    configFilePath || path.join(process.cwd(), "dream-config.json");
 ...
 */
export function getDefaultConfig(): DreamConfig {
