...
export class BisociativeSynthesisTool {
     ...
     */
    selectRandomDomain(matrixA) {
        // Filter out domains that are too similar to matrixA
        const eligibleDomains = DOMAINS.filter(domain => !matrixA.toLowerCase().includes(domain.name.toLowerCase()) &&
    ...
    identifyPatterns(domain) {
        ...
        for (const pattern of ISOMORPHIC_PATTERNS) {
            ...
            for (const [exampleDomain, example] of Object.entries(pattern.examples)) {
                if (domain.toLowerCase().includes(exampleDomain.toLowerCase()) ||
            ...
            if (domain.toLowerCase().includes(pattern.name.toLowerCase()) ||
     ...
     */
    findIsomorphism(patternsA, patternsB, preferredPattern) {
     ...
     */
    createMapping(domainA, domainB, pattern) {
        const mapping = {};
     ...
     */
    createExplanation(domainA, domainB, pattern, mapping, bridgeConcept) {
        const firstExampleKey = Object.keys(pattern.examples)[0] || '';
        const secondExampleKey = Object.keys(pattern.examples)[1] || '';
        const firstExample = firstExampleKey ? pattern.examples[firstExampleKey] : 'structured relationships';
        const secondExample = secondExampleKey ? pattern.examples[secondExampleKey] : 'organized components';
        return `
